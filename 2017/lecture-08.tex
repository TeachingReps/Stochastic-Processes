...
\eq{
\lim_{t \to \infty} \Pr\{Y(t) \leq x\} &= \frac{1}{\mu}\int_{0}^{x}\bar{F}(t)dt.
}
\end{prop}
\begin{proof}
...
That is, we consider an alternative renewal process $W$ such that
\eq{
W(t) &= 1\{Y(t) \leq x\}.% = 1\{S_{N(t)} \geq t-x\}
}
%This is an alternating renewal process with finite probability of off times being zero. 
%Further, we can write the $n$th on time $Z_n$ for this renewal process as
%\eq{
...
%From limiting on probability of alternating renewal process, we get 
%\eq{
%\lim_{t \to \infty} \Pr\{Y(t) \leq x\} &= \lim_{t \to \infty}P\{W(t) = 1\} = \frac{\E \min\{x,X\}}{\E X} = \frac{1}{\mu}\int_{0}^{x}\bar{F}(t)dt.
%}
\end{proof}
...
\eq{
\lim_{t\to\infty}\E Y(t) &= \int_{0}^{\infty}\lim_{t\to\infty}P\{Y(t) > x\}dx = \frac{1}{\mu}\int_{0}^{\infty}\int_{x}^{\infty}\bar{F}(t)dt = \frac{1}{2\mu}\int_{0}^{\infty}\bar{F}(t)dt^2 =  \frac{1}{2\mu}\int_{0}^{\infty}t^2dF(t).
}
...
\eq{
\lim_{t\to \infty}\frac{1}{t}\int_0^t Y(u) du &= \frac{\E X^2}{2\mu}.
}
\end{lem}
\begin{proof}
...
\eq{
\int_0^t Y(u) du &= \frac{1}{2}\sum_{n=1}^{N(t)}X_n^2 + \int_{S_{N(t)}}^t(S_{N(t)+1}-u)du.
}
In particular, one can write the following
\eq{
\frac{\sum_{n=1}^{N(t)}X_n^2}{2N(t)}\left(\frac{N(t)}{t}\right) &\leq \frac{1}{t}\int_0^t Y(u) du \leq \frac{\sum_{n=1}^{N(t)+1}X_n^2}{2(N(t)+1)}\left(\frac{N(t)+1}{t}\right).
}
Result follows from strong law of large number by taking limits on both sides. 
%It can also be established that  since, for each sample path $\{Y(t)= y(t)\}$ and sample values $\{x_i,i\in\N\}$, we have $\int_0^ty(\tau)d\tau = \frac{1}{2}\sum_{i=1}^{n(t)}x_i^2+\int_{\tau=s_{n(t)}}^t y(\tau)d\tau$.
\end{proof}
...
That is, %for a renewal process with inter-renewal duration distribution $F$ 
\eq{
P\{X_{N(t)+1} >x\} \geq \bar{F}(x). 
}
\end{thm}
%\begin{proof} 
%Clearly, $X_{N(t)+1}$ is a regenerative process with the regenerative sequence being the renewal instants. 
%We define kernel function for $x < t$ as 
%\eq{
%K(t) &= P\{X_{N(t)+1} > x, S_1 > t\} = P\{S_1 > x\vee t\} = P\{S_1 > t\} = \bar{F}(t).
%}
%Then, in terms of renewal function $m$ and the kernel function $K(t)$, 
%one can write the complementary distribution of $X_{N(t)+1}$ using the solution to the renewal equation as
%\eq{
%P\{X_{N(t)+1} >x\} &= \bar{F}(t) + \int_{0}^{t}\bar{F}(t-y)dm(y) = (\bar{F} + \bar{F}\ast m)(t) = 1+ m(t) - (F + F \ast m)(t).
%}
%Recall, $F +  F \ast m = m$, hence we have
%\eq{
%P\{X_{N(t)+1} >x\} &= 1 + m(t) - \int_{0}^{t}\bar{F}(t-y)dm(y).
%}
%\end{proof}
\begin{proof}
...
\eq{
\E f(X_{n+1})g(X_{n+1})/\E g(X_{n+1}) \geq \E f(X_{n+1}) = \bar{F}(x).
}
...
That is, %for a renewal process with inter-renewal duration distribution $F$ 
\eq{
\lim_{t \to \infty}P\{X_{N(t)+1} >x\} \geq \bar{F}(x). 
}
\end{lem}
\begin{proof}
...
%during a cycle (if the renewal interval is greater than $x$), or totally OFF otherwise. Formally, 
...
\eq{
\E W(t) &= P\{X_{N(t)+1} > x\} = P\{\text{ on at time } t\}.
}
%Now we have,
%\begin{align*}
...
\begin{align*}
\lim_{t\to \infty}\Pr\{X_{N(t)+1}>x\} %&= \frac{\E Z_n}{\mu} \\
&= \frac{\E X1\{X>x\}}{\E X}. %= \frac{\int_{x}^\infty y dF(y)}{\mu}. %&\geq \Pr[X_1\geq x]. 
\end{align*}
