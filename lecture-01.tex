...
\begin{defn} A  collection of random variables $\{X_t \in S: t \in T\}$ each defined on the same probability space $(\Omega, \F, P)$ is called a \textbf{random process} for an arbitrary index set $T$ and arbitrary state space $S$. 
\end{defn}
...
However, $T$ doesn't have to be time, it can be indicating space, and then the stochastic process is spatial process. 
State space $S$ can also be countable or uncountable. 
Further, $S$ can be uncountable in space and $T$ in time, leading to spatio-temporal processes. 
