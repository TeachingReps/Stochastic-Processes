...
%\end{defn}
%\begin{defn} 
A counting process $\{N(t), t\geqslant 0\}$ has \textbf{independent increments} if %for any $k_j\in \N_0, j \in [n]$
...
We write 
\eq{
\{A(s) > x\} &= \bigcup_{n \in \N_0}\{S_n < s - x, S_{n+1} > s, N(s) = n\} = \bigcup_{n \in \N_0}\{S_n < s - x, S_{n+1} > s\},\\
\{Y(s) > y\} &= \bigcup_{n \in \N_0}\{S_n \leq s, S_{n+1} > s + y, N(s) = n\} = \bigcup_{n \in \N_0}\{S_n \leq s , S_{n+1} > s+y\}.
}
