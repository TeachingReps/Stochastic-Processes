...
\end{align*}
%where the second equality follows from the fact that the expectation of a random variable being represented in terms of the \emph{ccdf} of the corresponding random variable. 
...
Note that since $X_i$'s are \emph{iid}, so are $\bar{X}_i$'s. % (which will be evident from the proof of the distribution function of the number of arrivals till time t). 
