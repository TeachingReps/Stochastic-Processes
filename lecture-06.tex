...
\begin{flalign*}
\Pr\{X_{N(t)+1}>x\}&= \Pr\{X_{N(t)+1}>x, S_{N(t)}=0\}+\Pr\{X_{N(t)+1}>x,  S_{N(t)}>0\}\\
&= \Pr\{X_{N(t)+1}>x, S_{N(t)}=0\} + \int_{0}^t\Pr\{X_{N(t)} > x | S_{N(t)} = y\}dm(y)\bar{F}(t-y).
%&= P(X_2 >x)\\
%&= \bar{F}(x).
\end{flalign*}
Since $\Pr\{X_{N(t)+1}>x | S_{N(t)}=y\} = \Pr\{X>x | X>y\} = \frac{\bar{F}(x)}{\bar{F}(y)}\geq \bar{F}(x)$, we have
\begin{flalign*}
\Pr\{X_{N(t)+1}>x\}\geq  \bar{F}(x).
\end{flalign*}
To obtain the limiting distribution of $X_{N(t)+1}$, consider an alternating renewal process for which the ON time is the total time of the cycle if that time is greater than $x,$ and zero otherwise. The system is either totally ON during a cycle (if the renewal interval is greater than $x$), or totally OFF otherwise. Then,
\begin{align*}
...
\begin{align*}
\lim_{t\to \infty}\Pr\{X_{N(t)}>x\} &= \frac{\E[\text{on time in cycle}]}{\mu} \\
&= \frac{\E[X|X>x]\bar{F}(x)}{\mu}\\
