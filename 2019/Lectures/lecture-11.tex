...
\indent \textbf{Example:} Consider a slotted queuing system in which one service time is equal to one slot. Let $q_k$ be the queue length at the end of $k^{th}$ slot. Let $A_k$ denote the number of arrivals in the $k^{th}$ slot. Then, $q_{k+1} = (q_k -1)^+ + A_{k+1}$ and for $i>0$
\begin{align*}
...
{\color{red}\indent Consider $f(i) = i$. Let $S_0 = \{0\}$. For $i> 0$, $\E[f(q_1)|q_0 = i] - f(i) = \E[A_1] -1 $. Thus, we see from case $(1)$ in Prop \ref{prop:markov_chain_class_tests}, if $\E[A_1] \leq 1 $, $\{q_k\}$ is recurrent. From $(2)$ in Prop \ref{prop:markov_chain_class_tests}, we have positive recurrence if $\E[A_k] < 1$.} \\
...
\end{align*}
under stationarity is a necessary and sufficient condition. 
\begin{proof}
