...
\textbf{Strong Markov property:} If $\tau$ is a stopping time and $\tau < \infty$ with probability $1$, then Markov chain has strong Markov property if
\begin{align*}
\P\{X_{\tau} = j|X_{\tau-1} = i, X_{\tau-2} = i_{\tau-2},\dots,X_1 = i_1,X_0 = i_0\} = \P\{X_1 = j|X_{0} = i\}. \ \forall n \in \mathbb{N} \text{ and }i,j \in S.
\end{align*}
\begin{proof}
{\color{red} Proof for strong Markov property: This proof might not hold in general case. I am assuming homogenous MC.}
\begin{align*}
...
\textbf{Communicating classes:} A state $j$ is said to be \textit{reachable} from state $i$ if there exists an $n$ such that $P^n(i,j) > 0$. We denote this by $i\rightarrow j$. If $i\rightarrow j$  and $j \rightarrow i$, we say that states $i$ and $j$ are \textit{communicating states} and denote this by $i \leftrightarrow j$. A subset $A$ of state space is called \textit{closed} if for all $j \in A^c$, and $i \in A$, $j$ is not reachable from $i$. A subset $A$ of state space is called a \textit{closed communicating set} if it is closed and $i \leftrightarrow j, \forall i,j \in A$. Communication is an equivalence relation. A set with only one communication class is called an \textit{irreducible} set.
...
\end{proof}
\end{prop}
{\color{red}If $i$ is transient and $j$ is recurrent, then as the above example shows, $i \rightarrow j$ is possible (as above example shows) but $j \rightarrow i$ is not possible, as we now show. If $i\rightarrow j$, then $j \rightarrow i$ is ruled out by Prop \ref{prop:rec_is_class_property}. If $i\rightarrow j$ is not true,  but $j \rightarrow i$ is true, then there is $m$ such that $P^m( j, i) >0$ without $j$ visiting itself. But then, $P_j(\tau_j= \infty) \geq P^m(j,i) >0$. Thus $j$ will not be recurrent. \\
...
\lim_{n\rightarrow \infty} P^n(j,j) = \frac{d}{\mu_{jj}} &&\text{(when $j$ has period $d> 1$).} 
\end{align*}
If the initial state $i$ is positive recurrent, then visits to state $i$ are regeneration epochs and we have 
\begin{align*}
