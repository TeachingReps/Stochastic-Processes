...
\eq{
\Pr\{X(s+t) = j | X(u)=i, ~ u \in [0,s]\},~s, t \geq 0.
}
...
%\end{defn}
%\begin{defn}
The $n$th \textbf{sojourn time} of a stochastic process $\{X(t), t \geqslant 0\}$ remaining in the same state is %in any state $i \in I$ are 
\begin{align*}
...
%\end{align*}
\eq{
\Pr\{T_n > s+t |T_n > s\} &= P_{X(S_{n-1}),X(S_{n-1})}(s,s+t) = P_{X(S_{n-1}),X(S_{n-1})}(0,t) = P\{T_{n} > t\}.
