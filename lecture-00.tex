...
\input{header}
\title{Lecture 1: Introduction to Stochastic Processes}
\author{}
...
\begin{align*}
P(\cup_{n}A_n) = \sum_nP(A_n).
\end{align*}
\end{enumerate}
\end{defn}
There is a natural order of inclusion on sets through which we can define monotonicity of probability set function $P$. To define continiuity of this set function, we need to define limits of sets. 
...
\begin{xalignat*}{3}
&\lim\sup_nA_n = \cap_n\cup_{k \geq n}A_k, && \lim\inf_nA_n = \cup_n\cap_{k \geq n}A_k.
\end{xalignat*}
...
\begin{exmp}[Queues] Queues are a complex stochastic process and consist of two stochastic processes arrival and service, coupled through a buffer. For a discrete arrival case, arrival process can be characterized by the time epochs of discrete arrivals, denoted $\{A_n: n \in \N \}$. Service of each arrival can be considered to be a random amount of time, $\{S_n: n \in \N\}$. If a queue can only be served at a fixed rate, then arrivals would have to wait in the buffer till their service time. Queues have applications in operations research, industrial engineering, telecommunications networks, among others. 
\end{exmp}
