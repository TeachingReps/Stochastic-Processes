...
\begin{defn}[Inter-arrival Times] Let $\{X_i: i \in \mathbb{N}\}$ be a sequence of non-negative \emph{iid} random variables with a common distribution $F$, with 
	\begin{enumerate}
...
\begin{defn}[Renewal Instants] Let $S_n$ denote the time of $n^{\mathrm{th}}$ renewal, and assume $S_0 = 0$. Then, we have
	\begin{align*} 
...
\begin{proof}
	...
	%0 ------- S_{N(t)}---t ---- S_{N(t)+1} ------------
	\begin{figure}
		\includegraphics[width=\linewidth]{lecture_5_fig_1.png}
	\end{figure}
